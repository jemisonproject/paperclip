...
## Step 6.5 — Install the Koiomi engineering skill

Skills are how OpenClaw agents learn behaviors specific to a team. The Paperclip skill handles Paperclip API interaction; you also need the Koiomi-specific engineering skill (branch naming, where repos live, PR conventions). It's tracked in the repo so you can install it on your laptop the same way Juan did:

```bash
# Make sure you have the paperclip repo cloned at ~/.openclaw/workspace/Koiomi/paperclip
# (Step 5 of OpenClaw onboarding already does this, but verify)
ls ~/.openclaw/workspace/Koiomi/paperclip/openclaw-skills/koiomi-engineering/SKILL.md

# Copy the skill into OpenClaw's skills directory
mkdir -p ~/.openclaw/skills/koiomi-engineering
cp ~/.openclaw/workspace/Koiomi/paperclip/openclaw-skills/koiomi-engineering/SKILL.md \
   ~/.openclaw/skills/koiomi-engineering/SKILL.md

# Verify
ls -la ~/.openclaw/skills/
```

If the skill file ever changes in the repo, re-run the `cp` to pick up updates.

> **Note for Juan / admin:** the skill source of truth is `openclaw-skills/koiomi-engineering/SKILL.md` in this repo. Edit there, commit, push, and have teammates re-`cp` after `git pull`.
