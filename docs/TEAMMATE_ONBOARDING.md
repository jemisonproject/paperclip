...
## Phase 3 — Optional: QA automation pipeline

The QA agent runs on the NAS (not on your PC). It picks up `in_review` tickets, visually verifies changes using Playwright, and either merges to `main` or creates bug tickets.

**This is set up once by Juan on the NAS.** See `docs/QA_AGENT_SETUP.md` for the full NAS setup. As a teammate, you don't need to do anything for QA — just set your tickets to `in_review` when ready and the QA agent handles the rest.
